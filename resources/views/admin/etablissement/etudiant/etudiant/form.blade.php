...
<div class="form-group {{ $errors->has('lieu_nais') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="lieu_nais" type="text" id="lieu_nais" value="{{ isset($etudiant->lieu_nais) ? $etudiant->lieu_nais : ''}}" required>
...
<div class="form-group {{ $errors->has('utilisateur') ? 'has-error' : ''}}">
    <label for="utilisateur" class="control-label">{{ 'Utilisateur' }}</label>
     <select class="select2 select2-success form-control" data-dropdown-css-class="select2-success" name="utilisateur" required>
        @foreach($user as $item)
            <option value="{{$item->id}}" @if(isset($etudiant->utilisateur) && $etudiant->utilisateur === $item->id ) selected @endif>{{$item->name}} 
            </option>
        @endforeach
    </select>
